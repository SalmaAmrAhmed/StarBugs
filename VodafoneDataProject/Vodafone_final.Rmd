...
ggplot() + geom_histogram(data = train_contract_ref_imputed, aes(x=as.numeric(rp.hours), y=..density..), colour = "gray", fill = "white", binwidth = 2)  + geom_density(data = filteredColumns, aes(x = as.numeric(rp.hours)), alpha = .2, fill = "magenta") + ggtitle("Total number of reports Vs rush hours")



qplot(TARGET, data=train_contract_ref_imputed, fill=GENDER, geom="bar")
qplot(GENDER, data=train_contract_ref_imputed, fill=factor(TARGET))
...
ALL with AGE                                               | sub_age.csv                           |
ALL with GENDER                                            | sub_gender.csv                        |
ALL with VALUE_SEGMENT                                     | sub_valuesegment.csv                  |
ALL with AGE, GENDER                                       | sub_age_gender.csv                    |
ALL with AGE, GENDER, VALUE_SEGMENT                        | sub_age_gender_valuesegment.csv       |
ALL with HANDSET                                           | sub_handset.csv                       |
ALL with AGE, GENDER, VALUE_SEGMENT, HANDSET_NAME          | sub_all.csv                           |
...
Model Attributes                                           | Submission File Name                  | Accuracy
-----------------------------------------------------------| --------------------------------------| --------
ALL, WEEKEND_CONSUMPTION, WEEKDAY_CONSUMPTION, ROAMUNG     | sub_weekend_weekday.csv               | 
...
//Load the feature selection photos:
```{r}
