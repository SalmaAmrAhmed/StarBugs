...
p2 <-  geom_bar(data = age_of_Med_low, aes(x=age_of_Med_low$contract_ref_filtered.AGE.contract_ref_filtered.VALUE_SEGMENT....), fill = "deepskyblue1 ", alpha = 0.4 ) + ggtitle("Age of Med Low Value segment") + xlab("Age") 
...
train_roaming <- left_join(train_9_10, local_sessions_count, BY = "CONTRACT_KEY")
train_roaming <- left_join(train_roaming, local_consumption, BY = "CONTRACT_KEY")
train_roaming <- left_join(train_roaming, roaming_consumption, BY = "CONTRACT_KEY")
train_roaming <- left_join(train_roaming, roaming_sessions_count, BY = "CONTRACT_KEY")
...
train_roaming$ROAMING_USAGE <- train_9_10$ROAMING_USAGE/(1024*1024)
train_9_10$ROAMING_USAGE <- as.integer(train_roaming$ROAMING_USAGE)
...
weekends <- read.csv("weekend.csv")
workdays <- read.csv("working_day.csv")
...
predictedFold <- data.frame(predict(model,  roam_week[1:13], type = "class"))
auc(as.numeric(roam_week$TARGET), as.numeric(predictedFold$predict.model..roam_week.1.13...type....class..))
...
train_6_10 <- left_join(train_6_10, local_s, BY = "CONTRACT_KEY")
train_6_10 <- left_join(train_6_10, local_c, BY = "CONTRACT_KEY")
train_6_10 <- left_join(train_6_10, roaming_c, BY = "CONTRACT_KEY")
train_6_10 <- left_join(train_6_10, roaming_s, BY = "CONTRACT_KEY")
...
train_7_10 <- left_join(train_7_10, local_s, BY = "CONTRACT_KEY")
train_7_10 <- left_join(train_7_10, local_c, BY = "CONTRACT_KEY")
train_7_10 <- left_join(train_7_10, roaming_c, BY = "CONTRACT_KEY")
train_7_10 <- left_join(train_7_10, roaming_s, BY = "CONTRACT_KEY")
...
train_8_10 <- left_join(train_8_10, local_s, BY = "CONTRACT_KEY")
train_8_10 <- left_join(train_8_10, local_c, BY = "CONTRACT_KEY")
train_8_10 <- left_join(train_8_10, roaming_c, BY = "CONTRACT_KEY")
train_8_10 <- left_join(train_8_10, roaming_s, BY = "CONTRACT_KEY")
...
Where we have done ensembling on all the previously mentioned trained models and the submission files of the test data, using the "Kaggle_avg.py" script [link](https://github.com/MLWave/Kaggle-Ensemble-Guide/blob/master/kaggle_avg.py),  

The Below JavaScript chart shows how each submission file contributed in our final submission.
![](spider.png)
